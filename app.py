...
def voice_reply():
    ...
    # ---- Memory Reset if Session Stale ----
    if not hasattr(voice_reply, "last_interaction"):
        voice_reply.last_interaction = time.time()

    if time.time() - voice_reply.last_interaction > 300:
    ...
    def detect_question_intent(text):
        ...
        return any(keyword in text.lower() for keyword in informational_keywords)

    def detect_short_affirmation(text):
        affirmations = ["yes", "yeah", "yep", "right", "correct", "that's right", "exactly", "sure", "of course"]
        return text.strip().lower() in affirmations

    def soft_transition_if_info(user_input, reply):
        informational_keywords = ["tell me", "can you tell me", "explain", "what is", "give me the information"]
        if any(keyword in user_input.lower() for keyword in informational_keywords):
            return f"Since you asked for information, here’s a brief explanation: {reply}"
        return reply
            ...
            gpt_response = openai.ChatCompletion.create(
                ...
                messages=[
                    {"role": "system", "content": (
                        ...
                        "If the user clearly asks for advice (using words like 'what should I do?', 'can you help me fix this?'), then gently offer one encouraging, non-overwhelming suggestion. "
                        "If the user requests specific information (using words like 'tell me', 'can you tell me', 'give me the information'), then offer detailed information based on bot. "
    ...
    final_reply = soft_transition_if_info(user_input, final_reply)
